...
describe("confirm dose event route", () => {
  ...
  it("returns 401 when the request is unauthenticated", async () => {
    ...
    const response = await POST(createRequest({ confirmationNote: "a".repeat(501) }), createContext());
  ...
  it("returns 403 when VIEWER tries to confirm a dose", async () => {
    prismaMocks.findFirst.mockResolvedValueOnce({ id: "dose-1", patientId: "patient-1" });
  ...
  it("uses the signed-in local user id when confirming a dose", async () => {
    const updatedDoseEvent = {
      id: "dose-1",
      status: "TAKEN",
      confirmationNote: "Taken",
